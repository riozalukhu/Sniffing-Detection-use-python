...
# Fetch first 20 logs from the database
c.execute("SELECT * FROM log LIMIT 20")
logs = c.fetchall()
...
# Function to slide data
def slide_data():
    # Remove existing logs from the treeview
    tree.delete(*tree.get_children())

    # Get the last timestamp from the logs displayed, if any
    last_timestamp = None
    children = tree.get_children()
    if children:
        last_timestamp = tree.item(children[-1])["text"]

    # Fetch next 20 logs from the database that have a timestamp greater than the last timestamp displayed
    c.execute("SELECT * FROM log WHERE timestamp > ? LIMIT 20", (last_timestamp,))
    logs = c.fetchall()
